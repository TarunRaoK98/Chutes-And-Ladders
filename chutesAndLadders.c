...
/* Queue data structure, used as a temporary memory resource */
...
{	
	...
	/* initializing Queue */
	QueueItem qi = {0, 0, 0}; 		// begins at Vertex 0 and distance is 0
	...
	{
		qe = dequeue(sequence);		// Current Vertex
		int vertex = qe.vertex; 

		if (vertex >= SIZE-1)		// if current Vertex is last Vertex , end
		    break;
		
		QueueItem max;		

		for (int j=1; j<= DICE && vertex+j<SIZE; j++)	// j represents dice roll values from 1 to 6
			
		{
			QueueItem temp;
			temp.vertex = vertex+j;
			temp.dice = j;
			temp.distance = qe.distance + 1;				
			...
			if(temp.vertex >= max.vertex)	//reason for shortest path
				max = temp;
		}
...
/* Initialize chute and ladder mappings */
...
/* Utility functions for Vertices */
...
void createBoard (Vertex* board[], int length)
{
	...
	for (int i = 0; i < length; i++)
		for(int j = 1; j <= DICE && ((i+j) < length); j++)
